...
Test(about_control_statements, ifs)
{
     ...
     * parenthesis
     * is evaluated. The only difference from Java is the lack of a boolean
     * type.
...
Test(about_control_statements, switch_block)
{
     ...
     *
     * The switch will 'fall through' without the 'break' keyword, meaning that
     * it
     * will do
...
Test(about_control_statements, loops)
{
     ...
     *
     * This brings another dicussion for the 'break' statement: if the control
     * flow reaches
     * that statement inside of a loop, than it will stop the execution of that
     * loop.
     */
    ...
    /* The for loop should also feel familar. It possess three statements inside
     * of its parenthesis:
     * the first is done before the first iteration, the second is the check to
     * stop iteration, and
     * the third is done after every iteration.
     *
     * We will write a for loop that does the same thing as the while loop in
     * the
     * previous example...
     */
...
{
    /*
     * The goto at the assembly level is an unconditional branch, 'j' or 'b'
     * from
     * MIPS.
    ...
    var = 50;
/* The label syntax is some name (the same as a variable name) followed by a
 * colon. */
label:
    cr_assert_eq(
