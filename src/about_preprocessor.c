...
Test(about_preprocessor, conditional_defines) {
     ...
     * appear in the file. For the next test, define MOVE_ME in a place such that it
     * will pass.
     */
...
Test(about_preprocessor, macro_concatination){
     ...
     * the '##' into one long token. As can be seen in the macro THIS_IS, it will
     * be replaced with one of the macros defined right above it.
     */
...
Test(about_preprocessor, variadic_macros) {
     ...
     * replaces the input argument list denoted by '...' where '__VA_ARGS__' is in the macro.
     *
     * It even includes the commas.
     */

    /* To pass this test, put the correct argument into the macro call.
     * Note that the macro takes advantage of stringizing. Your argument is not going to be a string.
     */
