...
Test(about_linked_lists, helpful_pointers) {
  ...
  cr_assert_eq(sizeof(struct _linked_list), TODO, "How much memory does it occupy?");
  ...
  cr_assert_eq(list.head->data, TODO, "What is the data contained in the head?");

  cr_assert_eq(list.tail, TODO, "Where is tail pointing to?");
  cr_assert_eq(list.tail->data, TODO, "What is the data contained in the tail?");
}
...
Test(about_linked_lists, doubly_linked_list) {
   ...
   *
   * We can reduce deletion time because we can easily modify the prev/next pointers
  ...
  cr_assert_eq(sizeof(struct dbl_linked_list), TODO, "How much memory does it occupy?");
