...
Test(c_koans, struct_basics) {
    ...
    /* Since we are using sizeof on p1, we do not have to enclose it in parenthesis */
    cr_assert_eq(sizeof p1, TODO, "What is the size of our two ints?");

    /* We can initialze our structs statically, with similar syntax to array initialization */
    /* The members are initialized in the order that they have been declared in the struct */
...
Test(c_koans, structs_and_functions_and_pointers) {
    ...
    cr_assert_eq(success, TODO_NZ, "If the operation succeeded, what is the return value?");
...
Test(c_koans, arrays_of_structs) {
    ...
    /* For arrays of structs, the size of the struct must be concious of the
    ...
    cr_assert_eq(sizeof (struct s2), TODO, "What is the size of the padded struct?");

    /* Compilers will have special extensions to allow for the packing of structs.
...
Test(c_koans, self_referential_structs) {
    ...
    cr_assert_eq(sv1.s->i, TODO, "What is the value of the nested struct's value i?");
}
